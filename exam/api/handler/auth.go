...
func (h *Handler) UserRegisterCreateConfirm(c *gin.Context) {
	...
	if err := check.CheckEmail(loginReq.Mail); err != nil {
		handleResponse(c, h.Log, "Email address does not exist"+loginReq.Mail, http.StatusBadRequest, err.Error())
		return
	}
	if err := check.ValidatePassword(loginReq.Password); err != nil {
	...
	if err != nil {
		handleResponse(c, h.Log, "password error hashing", http.StatusUnauthorized, err)
	}
	...
	if err != nil {
		handleResponse(c, h.Log, "error", http.StatusUnauthorized, err)
		return
	}
...
func (h *Handler) UserLoginOtp(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&loginReq); err != nil {
		handleResponse(c, h.Log, "error while binding bodyTOJSON", http.StatusBadRequest, err)
		return
	}
	...
	handleResponse(c, h.Log, "Otp sent successfully", http.StatusOK, "")
}
...
func (h *Handler) UserLoginOtp2(c *gin.Context) {
	...
	handleResponse(c, h.Log, "Succezs", http.StatusOK, loginResp)
...
func (h *Handler) Forgetpassword2(c *gin.Context) {
	...
	if err != nil {
		handleResponse(c, h.Log, "err", http.StatusUnauthorized, err)
		return
	}
