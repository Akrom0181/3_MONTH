...
func (a authService) UserLogin(ctx context.Context, loginRequest models.UserLoginRequest) (models.UserLoginResponse, error) {
	...
	if err != nil {
		a.log.Error("error while getting User by login", logger.Error(err))
...
func (a authService) UserRegister(ctx context.Context, loginRequest models.UserRegisterRequest) error {
	...
	msg := fmt.Sprintf("Your otp code is: %v, for registering User. Don't give it to anyone", otpCode)

	fmt.Printf("Your otp code is: %v, for registering User. Don't give it to anyone", otpCode)
...
func (u authService) UserRegisterCreateConfirm(ctx context.Context, User models.LoginUser) (models.UserLoginResponse, error) {
	...
	if !ok {
		u.log.Error("ERROR in service layer while creating user", logger.Error(errors.New("Failed to convert OTP code to string")))
		return resp, errors.New("Failed to convert OTP code to string")
	}

	if OTPCODEStr != User.MailCode {
		u.log.Error("ERROR in service layer while creating user", logger.Error(errors.New("The code you entered is not the same as the code sent to your gmail address")))
	...
	if err != nil {
		u.log.Error("ERROR in service layer while creating user", logger.Error(err))
		return resp, err
	}
